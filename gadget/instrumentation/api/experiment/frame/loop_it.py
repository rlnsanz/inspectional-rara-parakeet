...
class loop_it:
    ...
    def new(iter8r, text, name, line_no):
        ...
        print(f"{stringify.vertical_prefix_string()}LOOP_IN {name}: "
        ...
        i = 0
        for each in iter8r:
            print(f'{stringify.vertical_prefix_string()}****LOOP_ITERATION {name}@{i}: '
    ...
    def pop():
        ...
        state.depth -= 1
        print(f"{stringify.vertical_prefix_string()}LOOP_OUT {d['name']}: loop {d['name']} ended")
        state.lsn += 1