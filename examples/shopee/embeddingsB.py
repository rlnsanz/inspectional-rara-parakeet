...
data["1st_deliver_attempt"] = pd.to_datetime(
    data["1st_deliver_attempt"], unit="s"
).dt.date
data["2nd_deliver_attempt"] = data["2nd_deliver_attempt"].replace(
    np.nan, 0
)  # change nan to 0 or else can't be process
data["2nd_deliver_attempt"] = pd.to_datetime(
    data["2nd_deliver_attempt"], unit="s"
).dt.date
data.head()
...
data["1st_pick"] = np.busday_count(
    data["pick"], data["1st_deliver_attempt"], weekmask="1111110", holidays=holiday
)
data["2nd_pick"] = np.busday_count(
    data["1st_deliver_attempt"],
    data["2nd_deliver_attempt"],
    weekmask="1111110",
    holidays=holiday,
)
data.head()
...
submission = pd.DataFrame(
    {"orderid": data["orderid"], "is_late": data["is_late"].apply(int)}
)
submission
